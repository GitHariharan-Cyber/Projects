...
class PaperCompliantICSAnalyzer:
    ...
    def run_complete_pipeline(self, arff_file_path):
        """Run complete pipeline: Discretization → Bloom Filter → LSTM Training"""
        print("="*80)
        ...
        self.discretizer.fit(data, use_clean_data=False)  # Use full data to avoid empty arrays
        signatures, feature_vectors, self.unique_signatures = self.discretizer.discretize_dataset(data)
        ...
        self.lstm_detector = self._train_lstm_model(feature_vectors, signatures, self.unique_signatures)
        
        # Step 4: System Integration Test
        print("\n4. SYSTEM INTEGRATION TEST")
        print("-" * 40)
        
        self._test_complete_system(feature_vectors, signatures)
        
        print("\n" + "="*80)
        print("COMPLETE SYSTEM TRAINED SUCCESSFULLY!")
        print("="*80)
    ...
    def _train_lstm_model(self, feature_vectors, signatures, unique_signatures):
        """Train LSTM model with paper-compliant architecture and parameters"""
        ...
        # Create sequences for training (10-packet windows)
        ...
        # Split data: 60% train, 20% validation, 20% test (as per paper)
        X_temp, X_test, y_temp, y_test = train_test_split(
            X_sequences, y_sequences, test_size=0.2, random_state=42, shuffle=False
        )
        X_train, X_val, y_train, y_val = train_test_split(
            X_temp, y_temp, test_size=0.25, random_state=42, shuffle=False  # 0.25 * 0.8 = 0.2
        )
    ...
    def _test_complete_system(self, feature_vectors, signatures):
        """Test the complete two-layer detection system"""
        print("Testing complete anomaly detection system...")
        
        if len(feature_vectors) < 111:
            print("Not enough data for testing")
            return
            
        # Create a test sequence
        test_sequence = feature_vectors[100:110]  # 10 consecutive packets
        actual_next_signature = signatures[110]
        
        print(f"Test sequence: Packets 100-109")
        print(f"Actual next packet signature: {actual_next_signature}")
        
        # Layer 1: Bloom Filter Detection
        bloom_anomaly, bloom_confidence = self.bloom_filter.package_level_detection(actual_next_signature)
        print(f"\nLayer 1 - Bloom Filter:")
        print(f"  Result: {'ANOMALY' if bloom_anomaly else 'NORMAL'}")
        print(f"  Confidence: {bloom_confidence:.3f}")
        
        # Layer 2: LSTM Time-series Detection
        lstm_anomaly, lstm_confidence, top_predictions = self.lstm_detector.time_series_anomaly_detection(
            test_sequence, actual_next_signature, top_k=4
        )
        
        print(f"\nLayer 2 - LSTM Network:")
        print(f"  Result: {'ANOMALY' if lstm_anomaly else 'NORMAL'}")
        print(f"  Confidence: {lstm_confidence:.3f}")
        print(f"  Top-4 predictions:")
        for i, (sig, prob) in enumerate(top_predictions):
            match_indicator = "✓" if sig == actual_next_signature else " "
            print(f"    {match_indicator} {i+1}. {prob:.3f} - {sig}")
        
        # Combined decision
        final_anomaly = bloom_anomaly or lstm_anomaly
        print(f"\nFinal Decision: {'ANOMALY DETECTED' if final_anomaly else 'NORMAL PACKET'}")
