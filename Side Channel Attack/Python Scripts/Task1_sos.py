...
def generate_key_candidates(trace_cube):
    """
    Compute energy per cycle (sum of squares) and generate candidate bits.
   
    """
    cycle_energy_matrix = np.sum(trace_cube ** 2, axis=2)  # shape: (slots × cycles)
...
# ===== MAIN EXECUTION =====
def main():
    print("=== Power Trace Key Analysis ===")
    ...
    # Generate candidates
    key_candidates, mean_energy, cycle_energy_matrix = generate_key_candidates(trace_cube)
    ...
    # Save outputs
    np.savetxt(OUTPUT_DIR / "key_candidates_bits.txt", key_candidates, fmt="%d")
    np.savetxt(OUTPUT_DIR / "best_deltas.txt", best_deltas, fmt="%.6f")
    np.savetxt(OUTPUT_DIR / "cycle_energy_matrix.txt", cycle_energy_matrix, fmt="%.6f")

    # Plot comparison
    plt.figure(figsize=(12, 6))
    plt.plot(best_deltas, "o-", label="Delta Best (computed)", linewidth=2, markersize=5)
    plt.plot(ref_deltas, "s--", label="Reference δ-values", alpha=0.7)
    plt.xlabel("Key Candidate Index")
    plt.ylabel("Correctness (%)")
    plt.title("Candidate Correctness - Sum of Squares Compression")
    plt.legend()
    plt.grid(True, linestyle=":")
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "delta_best_comparison.png")
    plt.show()

    print("\nSaved files in:", OUTPUT_DIR)
    print("  - key_candidates_bits.txt")
    print("  - best_deltas.txt")
    print("  - cycle_energy_matrix.txt")
    print("  - delta_best_comparison.png")
