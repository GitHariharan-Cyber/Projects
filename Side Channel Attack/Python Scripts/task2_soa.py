...
TRACE_START_INDEX = 2620250                 # Starting sample index for processing
...
def generate_key_candidates(cube_matrix):
    """
    ...
    """
# Energy per cycle = sum of absolute amplitudes over 625 samples
    cycle_energy = np.sum(np.abs(cube_matrix), axis=2)  # shape: (230, 54)

    # Average per cycle position across slots
    avg_per_cycle = np.mean(cycle_energy, axis=0)  # shape: (54,)

    # Candidate bits: 1 if below mean, else 0
    cand_bits = (cycle_energy <= avg_per_cycle).astype(np.int8).T  # (54, 230)

    return cand_bits, avg_per_cycle
...
def main():
    ...
    key_candidates, mean_cycle_energy = generate_key_candidates(slot_matrix)
    ...
    plt.title("Comparison-to-Mean Scalar Recovery (per cycle) using SOA compression method")
    plt.legend()
